<?php
	/************************************************************************/
	/* ajaxCRUD.class.php	v5.2                                            */
	/* ===========================                                          */
	/* Copyright (c) 2010 by Loud Canvas Media (arts@loudcanvas.com)        */
	...
	function showTable(){
        ...
        */
        ...
        if ($row_count > 0){
            ...
            foreach ($rows as $row){
                ...
                if ($this->delete || (count($this->row_button)) > 0){
                    ...
                    if ($this->delete){
                        $table_html .= "<input type=\"button\" class=\"editingSize delbtn\" onClick=\"confirmDelete('$id', '" . $this->db_table . "', '" . $this->db_table_pk ."');\" value=\"delete\" />\n";
                    }
        ...
        if ($this->add){
            ...
            if ($this->ajax_add){
                $add_html .= "<input class=\"editingSize btn\" type=\"button\" onClick=\"
            ...
            $add_html .= "</td><td><input style='float: right;' class=\"editingSize btn\" type=\"button\" onClick=\"$('#add_form_$this->db_table').slideUp('slow');\" value=\"Cancel\"></td></tr>\n</table>\n";
	...
	function makeAjaxEditor($unique_id, $field_name, $field_value, $type = 'textarea', $field_size = "", $field_text = "", $onKeyPress_function = ""){
        ...
        <span id=\"" . $prefield ."_edit\" style=\"display: none;\">
            <form style=\"display: inline;\" name=\"form_" . $prefield . "\" id=\"form_" . $prefield . "\" onsubmit=\"
                ...
                var req = '" . $this->ajax_file . "?ajaxAction=update&id=" . $unique_id . "&field=" . $field_name . "&table=" . $this->db_table . "&pk=" . $this->db_table_pk . "&val=' + document.getElementById('" . $input_name . "').value;
				sndUpdateReq(req);
				return false;
			\">";
            ...
            if ($type == 'text'){
                ...
                $return_html .= "<br /><input type=\"submit\" class=\"editingSize btn\" value=\"Ok\">\n";
			}

        $return_html .= "
			<input type=\"button\" class=\"editingSize btn\" value=\"Cancel\" onClick=\"
